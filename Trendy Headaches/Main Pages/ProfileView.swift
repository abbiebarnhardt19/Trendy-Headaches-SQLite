...
struct ProfileView: View {
    ...
    var body: some View {
                ...
                VStack {
                    ...
                    //not editing
                        ...
                        HStack(alignment: .top) {
                            ...
                            VStack {
                                VStack {
                                    CustomText(text: "Symptoms", color: accentColor, width: viewColumnWidth - 10, textAlignment: .center, multilineAlignment: .center, isBold: true)
                                    
                                    CustomList(items: symptoms, color: accentColor)
                                }
                                
                                VStack {
                                    CustomText(text: "Triggers", color: accentColor, width: viewColumnWidth - 10, textAlignment: .center, multilineAlignment: .center, isBold: true)
                                    
                                    CustomList(items: triggers, color: accentColor)
                                }
                                
                                VStack {
                                    CustomText(text: "Preventative Meds", color: accentColor, width: viewColumnWidth - 10, textAlignment: .center, multilineAlignment: .center, isBold: true)
                            ...
                            VStack(alignment: .center) {
                                VStack {
                                    CustomText(text: "Emergency Meds", color: accentColor, width: viewColumnWidth - 20, textAlignment: .center, multilineAlignment: .center, isBold: true)
                                    
                                    CustomList(items: emergencyMeds, color: accentColor)
                                }
                                
                                VStack {
                                    CustomText(text: "Security Question", color: accentColor, width: viewColumnWidth - 20, textAlignment: .center, multilineAlignment: .center, isBold: true)
                                    
                                    CustomList(items: [securityQuestion], color: accentColor)
                                }
                                
                                VStack {
                                    CustomText(text: "Color Theme", color: accentColor, width: viewColumnWidth - 20, textAlignment: .center, multilineAlignment: .center, isBold: true)
                                    
                                    CustomList(items: [themeName], color: accentColor)
                                }
                                
                                // button with options that appear when clicked
                                CustomFloatButton(accent: accentColor, background: backgroundColor, options: buttonNames, actions: buttonActions)
        ...
        .onAppear {
            ...
            //make dictionary for loop (foreign key)
            ...
            fetchForeignLists.forEach { param in
                var list = DatabaseManager.shared.getForeignKeyColumnValues( userId: userID, tableName: param.table, columnName: param.column, filterColumn: param.filterCol, filterValue: param.filterVal)
